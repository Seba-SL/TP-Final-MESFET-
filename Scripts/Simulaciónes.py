...
plt.plot(VGS, ID_completo, linewidth=3, label="Modelo Completo",color = "green" , linestyle = "--")
plt.xlabel(r"$V_{GS}$ [V]")
plt.ylabel(r"$I_D$ [A]")
plt.title("Característica de transferencia MESFET")
plt.grid(True)
plt.legend()
plt.show()
...
plt.ylabel(r"$I_D$ [A]")
plt.title("Región óhmica del MESFET")
plt.grid(True)
plt.legend()
plt.show()
