...
Llegando finalmente a (\ref{equation:obtenicon_VP_2}), conocida como tensión de \textit{pinchoff}:
\begin{equation}\label{equation:obtenicon_VP_2}
		 V_{p0} = \frac{a^2 \cdot q\cdot N_D}{2 \cdot \epsilon_s }
\end{equation}


Se define a su vez, la tensión umbral como:
...
Se simulara este efecto reemplazando la expresión de la velocidad de arrastre de (\ref{equation:J_corriente_deduccion_1}), por la expresión (\ref{equation:v_arr_enfuncion_de_E}) función del campo eléctrico aplicado y utilizando $v_{sat} = 6.8\cdot 10^6 \, \frac{cm}{s}$ junto con constantes de aproximación y la expresión del largo del canal (\ref{equation:L_Modulada}). La curva simulada sera la de salida, es decir la corriente $I_D$ en función de $V_{DS}$ ya que es la que es capaz de forzar el campo de saturación.
