...
Por lo tanto la corriente para $V_{DS} > V_{DS_{[SAT]}} $ respondería a la expresión (\ref{equation:ID_SAT_COMPLETA_3} ) multiplicada por un factor que ajuste la nueva longitud. Se define el parámetro de modulación de longitud de canal $\lambda $ para describir este comportamiento.
	
	\begin{equation}
		I_{D_{[SAT]} } ' = I_{D_{[SAT]} } \cdot \frac{L}{L'}  = I_{D_{[SAT]} } \cdot (1 + \lambda \cdot V_{DS})
	\end{equation}
