...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|c|c|}
		...
		Masa efectiva ${m}^*_n$ &$0,0067\cdot m_o$ & -- & -- & -- \\ \hline
		...
		Concentración  $N_D$ [cm$^{-3}$] & $2 \cdot 10 ^{15}$ & -- & -- & -- \\ \hline
...
El modo lineal o óhmico responde linealmente tal como lo haria un resistor y su corriente de \textit{drain} se rige por las condiciones de la expresión (\ref{equation: modo_lineal}).
...
\begin{minipage}[c]{0.5\textwidth}
	...
	\begin{tikzpicture}[
		...
		\fill[gray!20] (-3,0) rectangle (2.5,1.99);
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.45\textwidth}
		...
		\begin{equation}\label{equation:nivel_fermi_tipon}
			E_f = \frac{1,42}{2}\, eV + kT\cdot \ln\!\left(\frac{2\cdot 10^{15}}{1.79\cdot 10^{6}}\right) = 1,25 \; eV
		\end{equation}
...
Se definen la barrera de potencial entre el metal y el semiconductor $\Phi_B$ = ($\Phi_M - \chi$) y barrera de juntura $\Phi_{bi}$ = $\Phi_B - \frac{1}{q} (E_c - E_f)$, también se puede apreciar como la flexión determina la zona de vaciamiento $W_d$ en equilibrio definida como $W_d0$.
...
Unión de contacto óhmico: (**********modificar****)
\begin{figure}[H]
	\begin{tikzpicture}
		[
		>=Latex,
...
La puerta está polarizada tan inversamente que el ancho del canal se hace nulo $d_{eff} = a - W_d(x) = 0 \,$ ya que se cumple $W_d(x) = a \wedge W_d(x) > W_{d0}$, se define $V_P$ como la tensión $V_{GS}$ donde esto comienza a cumplirse. 

Se introduce la tensión $V_R =  V(x) - V_{GS} $ que es la responsable de aumentar la flexión en las bandas. Donde se define como referencia $V_S = 0 $ , por lo tanto $V(x) \in [0 , V_{DS}]$ y $V_{GS} = V_{G}$. A su vez al estar fuera del equilibrio el nivel de Fermi varia entre materiales, se introduce el concepto de cuasi-nivel de Fermi para cada nivel, $E_{f_{sc}}$ para el semiconductor y $E_{f_{m}}$ para el metal. 
...
En este régimen se cumple $V_{GS} > V_P \quad \wedge \quad V_{DS} > V_{DS_{[SAT]}} = V_P + V_{GS} - V_{bi}$, por lo tanto el canal es controlado por $V_{GS}$. 
Se puede observar como el ancho de la zona de vaciamiento se encuentra en el rango de $ W_{d0} < W_d(0) < a$ mientras que $ W_d(L) = a $ . A medida que  $V_{DS}$ aumenta más allá de  $V_{DS_{[SAT]}}$, la longitud de la región de empobrecimiento crece a lo largo del canal, pero el nivel de $I_D$ permanece igual, es por eso que en esta región $V_{DS}$ no incide en $I_D$. Los portadores atraviesan la región de empobrecimiento debido a el arrastre generado por el campo externo longitudinal.


\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{Img/diagrama_bandas_alolargodelcanal}
	\caption{Diagrama de bandas en $W_D(0) = W_{Ds}$ y $W_D(L) = W_{Dd}$ \parencite{mishra_semiconductor_2008}. }
...
\begin{tikzpicture}[
	...
	% EJE ENERGÍA
	% --------------------
	\draw[axis] (0,-0.5) -- (0,3.5) node[above] {Energía};
	\draw[axis] (0,0) -- (10,0) node[right] {Posición $x$};
	
	% --------------------
	% BANDA DE CONDUCCIÓN Ec(x)
	% --------------------
	\draw[band]
	(0,2.8)
	-- (3,2.6)
	...
	\node[annot] at (9.7,2.0) {$E_C(x)$};
	
	% --------------------
	% BANDA DE VALENCIA Ev(x)
	% --------------------
	\draw[band]
	(0,0.8)
	-- (3,0.6)
	...
	\node[annot] at (9.7,0.2) {$E_V(x)$};
	
	% --------------------
	% CUASI NIVEL DE FERMI ELECTRONES
	% --------------------
	\draw[level]
	(0,2.4)
	-- (3,2.2)
	...
	\node[annot] at (9.7,1.6) {$E_{Fn}(x)$};
	
	% --------------------
	% ANOTACIONES SOURCE / DRAIN
	% --------------------
	\node[annot, below] at (0,0) {Source};
	\node[annot, below] at (10,0) {Drain};
	...
	\draw[<->, thick]
	(5,1.8) -- (5,2.4);
	\node[annot, right] at (5.1,2.1) {$qV(x)$};
...
El dispositivo bajo conducción forma una densidad de corriente de arrastre de portadores mayoritarios $	J_{arr, n} $ en el eje \textit{x} , debido al campo aplicado $\xi$, de acuerdo a la expresión (\ref{equation:J_corriente_deduccion_1}):
...
Reemplazando $W(x)$ , por su expresión en (\ref{equation: W_d_polarizado}), se llega a .
...
\begin{equation}
	...
	\sqrt{
		...
		\left( \phi_{bi} - V_{GS} + V(x) \right)
	}
	\right]
	\left( - \frac{dV(x)}{dx} \right)
\end{equation}

Se puede operar la expresión para integrar ambos lados 
...
\begin{equation}
	...
	-
	\left(
	\frac{2 \epsilon }{q N_D}
	\left( V(x) + V_{bi} - V_{GS} \right)
	\right)^{1/2}
	\right]
	\, dV
\end{equation}

Finalmente se llega a la expresión completa en función de los parámetros de control y del circuito externo.
...
\begin{tcolorbox}[colback=black!5!white, colframe=black!75!black, title=Expresión de la corriente ID completa en función de los parámetros de control y del circuito externo]
		...
		\begin{equation}
				I_D
				=
				\frac{q \, \mu_n \, N_D \, Z \, a}{L}
				\left\{
				V_{DS}
				-
				\frac{2}{3}
				\frac{
					\left( V_{DS} + V_{bi} - V_{GS} \right)^{3/2}
					-
					\left( V_{bi} - V_{GS} \right)^{3/2}
				}{
					\left( \dfrac{e N_D a^2}{2 \epsilon_s } \right)^{1/2}
				}
				\right\}
		\end{equation}
...
Comunmente se define la conductancia de canal completo $g_o =\frac{q \, \mu_n \, N_d \, Z \, h}{L}  $ y ya conociendo la expresión de tensión $V_p = \frac{q N_D a^2}{2 \epsilon } $, la expresión se puede reescribir como:
