...
Una vez obtenida la expresión completa (\ref{equation:Expresion_Completa}) se puede comparar con el modelo clásico de Schockley (\ref{equation:modo_estrangulacion}), se puede apreciar que el modelo clásico es una buena aproximación a grandes rasgos pero a medida que se contemplan efectos no ideales esta deja de ser valida.  
...
Utilizando las expresiones de $I_D$ tomando como variable $V_{GS}$ y utilizando $V_{DS} = V_{DS_{[SAT]}}$ , se obtienen las curvas \ref{fig:curvatransferencia}:
...
Se puede contemplar como el modelo completo forma una curva tomando valores inferiores en relación del modelo clásico para el rango de estrangulación.



\subsection{Curva de salida}

Utilizando las expresiones de $I_D$ en función de la tensión de $V_{DS}$ para diferentes valores de la tensión de control $V_{GS}$, tanto el modelo clásico de Schockley como la expresión deducida en (\ref{equation:ID_SAT_COMPLETA_3}) ¸se obtienen las curvas de la figura \ref{fig:curvadesalida}:
...
Utilizando la relación $ {I_D}' = I_D\cdot \frac{v_{arr}(\xi)}{\mu_n \cdot \xi}$, siendo ${I_D}'$ la corriente contemplando la saturación de $v_{arr}$ se llega a la curva:
