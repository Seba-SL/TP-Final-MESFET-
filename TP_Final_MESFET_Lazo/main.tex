...
Se analiza el transistor MESFET (Metal Semiconductor Field Effect Transistor). Se aborda su estructura, parámetros característicos, aplicaciones típicas, modos de operación, así como su modelo clásico y su modelo completo partiendo de la condición de neutralidad. Se presentan diagramas de bandas para cada modo de operación, curvas características y se discuten los efectos no ideales que influyen en su comportamiento real.

\section{Introducción al dispositivo}

El transistor MESFET es un dispositivo de tres terminales: fuente, drenaje y compuerta. Su estructura forma un canal por el cual fluyen los portadores desde el terminal fuente hacia el terminal de drenaje. La conductividad del canal es modulada por el potencial eléctrico aplicado en el terminal de compuerta. Pertenece a la familia de los FET (transistores de efecto de campo), con la diferencia de que no posee una capa de óxido aislante entre la compuerta y el sustrato como en el caso del MOSFET, característica que también comparte con el transistor JFET. Con respecto a este último, difieren en el material de la compuerta, ya que en el MESFET no es de material semiconductor, sino metálica, entre los metales utilizados se encuentran el aluminio, titanio, oro, níquel o platino entre otros.
...
Sobre el sustrato semi-aislante se encuentra la región activa del dispositivo, conformada por la capa de GaAs dopada ligeramente, en ella se forma el canal por el cual circulan los portadores mayoritarios cuando el dispositivo está en conducción.
...
\begin{figure}[H]
	...
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{Img/6W_MESFET_circuito_integrado.png}
		\caption{Arreglo integrado de MESFETs de GaAs \parencite{giannini2009high}.}
		\label{fig:giannini_MESFETs}
		\vspace{0.3cm}
		
		\includegraphics[width=0.8\linewidth]{Img/esquematico_de_paper}
	...
	\begin{minipage}{0.42\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{Img/Microscopio_MESFET_MISHRA.png}
		\caption{Sección transversal (arriba) y vista superior (abajo) de un MESFET de 0,1 $\mu m$ \parencite{mishra_semiconductor_2008}.}
...
\subsection{Aplicaciones típicas}
	...
	Es empleado en sistemas de comunicación por microondas, amplificadores, radiotelescopios hasta antenas parabólicas domésticas, sistemas satélitales y teléfonos celulares operando en frecuencias por encima de 3 GHz. 	
	 La tecnología MESFET es relevante en aplicaciones especializadas de alta potencia y alta temperatura, se suelen adoptar para frecuencias de hasta 18-20 GHz, mientras que la adopción de dispositivos de heterojunción (principalmente del tipo HEMT) es prioritaria para frecuencias de operación más altas \parencite{giannini2009high}.
...
En el informe se tomara como objeto de estudio un transistor MESFET formado por GaAs de canal n de empobrecimiento (D-MESFET), con silicio como impureza dopante en el canal y titanio como el metal de la compuerta a una temperatura de 300 K. A continuación se presentaran los diferentes parámetros típicos de referencia utilizados en el informe, en secciones posteriores se aclararan modificaciones convenientes si lo requiere el caso.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|c|c|}
		...
		Campo de saturación $\xi_{sat}$ [$\frac{kV}{cm}$] & $100$ & $100$ & -- & -- \\ \hline
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|l|}
		...
		Tensión umbral & $V_p$ & V & Tensión $V_{GS}$ para la cual el dispositivo comienza a conducir. \\ \hline
		Tensión de pellizco (pinch--off) & $V_{p0}$ & V & Tensión $V_{GS}$ para la cual el canal se estrangula por completo. \\ \hline
		Corriente de saturación & $I_{DSS}$ & A & Corriente de drenaje para $V_{GS} = 0$ (D-MESFET).\\ \hline
		Parámetro de modulación del canal & $\lambda$ & $V^{-1}$& Produce un efecto no ideal del transistor.\\ \hline
	\end{tabular}
\end{table}
...
Para el análisis del comportamiento del dispositivo se utiliza generalmente una nomenclatura que permite distinguir entre señales continuas y señales alternas en el tiempo. De este modo, cualquier magnitud eléctrica puede descomponerse en una componente continua (DC) y una componente alterna (AC). 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{Img/simbolo_mas_esquema}
	\caption{Simbología típica y esquema \parencite{Sharma_Metal_Semiconductor}. }
...
Se debe resaltar que estos modelos se cumplen para valores de tensión que no generen campos críticos que formen corrientes de ruptura y dentro de un rango de temperatura de operación ideal.

\subsection{Modo corte}

El modo corte sucede cuando la tensión $V_{GS}$ no supera la tensión $V_p$ o tensión umbral, y el transistor idealmente no tendrá corriente a través de la rama del \textit{drain}, tal como lo indica la expresion (\ref{equation:modo_corte}).

\begin{equation}\label{equation:modo_corte}
		V_{GS} \le V_p \quad \wedge \quad I_D = 0
\end{equation}
...
El análisis en profundidad parte de las bandas de energía de los materiales que conforman el dispositivo y como se comportan en las interfases entre ellos, en equilibrio termodinámico y bajo potenciales externos.
...
\begin{itemize}
	...
	\item $E_g = E_c - E_v$ : La energía de \textit{gap}, la diferencia entre la banda de valencia y de conducción.
\end{itemize}

\begin{comment}

Existen tres clasificaciones principales para los solidos: Aislantes, semiconductores y conductores, y sus diagramas de bandas típicos se encuentran en la figura \ref{fig:bandasindividuales}:

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{Img/Bandas_individuales}
...
Para obtener el campo eléctrico (\ref{equation:campo_electrico}), utilizamos la ley de Gauss ($	\xi = \frac{Q}{\epsilon_s}$):
...
Relacionando el campo eléctrico (\ref{equation:campo_electrico}) junto con el potencial (\ref{equation:potencial_electrostatico}) se puede despejar el ancho de la zona de agotamiento o vaciamiento en reposo y equilibrio térmico (\ref{equation: W_d_reposo}).
...
Al considerar la polarización del dispositivo se produce un potencial longitudinal $V(x) \in [0 ,V_{DS}]$ y el potencial externo trasversal al canal entre \textit{gate} y \textit{source} $V_{GS}$, que afectaran a la modulación de la zona de vaciamiento (\ref{equation: W_d_polarizado}), nombramos $ \phi_{bi} = V_{bi}$.

\begin{equation}\label{equation: W_d_polarizado}
	W_d(x) = \sqrt{\frac{2 \cdot \epsilon_s \cdot [  V_{bi} - V_{GS} + V(x)] }{q\cdot N_D}}
\end{equation} 

\subsection{Obtención de $V_{p0}$ y $V_{p}$}

Partiendo de (\ref{equation: W_d_reposo}) reemplazando $W_d(x) = a $ se obtiene la tensión para la cual el dispositivo comienza a conducir \ref{equation:obtenicon_VP}: 

\begin{equation}\label{equation:obtenicon_VP}
	W_d = \sqrt{\frac{2 \cdot \epsilon_s \cdot V_{p0} }{q\cdot N_D}} = a 
\end{equation} 

Llegando finalmente a (\ref{equation:obtenicon_VP_2}), conocida como tensión de \textit{pinchoff} y la tensión umbral como $V_{p}$:
\begin{equation}\label{equation:obtenicon_VP_2}
		 V_{p0} = \frac{a^2 \cdot q\cdot N_D}{2 \cdot \epsilon_s } \quad \wedge \quad 	V_{p} = V_{bi} - V_{p0}  
\end{equation}
...
Se puede apreciar que se forma la tensión de \textit{pinchoff} $ V_{p0}= \frac{a^2 \cdot q\cdot N_D}{2 \cdot \epsilon_s}$, llegando finalmente a la expresión (\ref{equation:obtension_VDS_SAT_4}):

\begin{equation}\label{equation:obtension_VDS_SAT_4}
	V_{DS_{[sat]}} = V_{p0} + V_{GS} -  V_{bi}
\end{equation} 


\section{Corriente}
\label{sec:modelo_completo}
...
\begin{equation}\label{equation:J_corriente_deduccion_1}
	J_{arr, n} = q\cdot n\cdot 	v_{arr}  =q \cdot n \cdot \mu_n \cdot \xi  =  q \cdot N_D \cdot \mu_n \cdot (- \frac{dV}{dx})
\end{equation}
...
Debido que la corriente se define positiva entrante al dispositivo, se cambia el signo de la expresión.
...
Se puede despejar $V_{p0}$ de la siguiente forma: 
\begin{equation}
  \sqrt{ \frac{2 \cdot \epsilon_s}{q\cdot N_D \cdot a^2}}	\int_0^{V_{DS}} \sqrt{ (V_{bi} - V_{GS} + V(x))} \; dV
\end{equation}

\begin{equation}
	\sqrt{ \frac{1}{V_{p0}}} \cdot \int_0^{V_{DS}} \sqrt{ (V_{bi} - V_{GS} + V(x))} \; dV
\end{equation}

\begin{equation}
	 \frac{1}{\sqrt{V_{p0}}} \cdot \frac{2}{3} \left[ (V_{bi} - V_{GS} + V_{DS} )^{\frac{3}{2}} - ( V_{bi} - V_{GS})^{\frac{3}{2}} \right]
\end{equation}
...
\begin{equation}
	I_D = g_o \cdot	\left\{ V_{DS}  - \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[ (V_{bi} - V_{GS} + V_{DS} )^{\frac{3}{2}} - ( V_{bi} - V_{GS})^{\frac{3}{2}} \right]\right\} 
\end{equation}


\begin{tcolorbox}[colback=black!5!white, colframe=black!75!black, title=Expresión de la corriente ID (modelo completo en función de los parámetros de control y del circuito externo)]


		\begin{equation} \label{equation:Expresion_Completa}
				I_D = g_o \cdot	\left\{ V_{DS}  - \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[ (V_{bi} - V_{GS} + V_{DS} )^{\frac{3}{2}} - ( V_{bi} - V_{GS})^{\frac{3}{2}} \right] \right\} 
		\end{equation}
	
	Valida para: $ V_{GS} > V_p \quad \wedge \quad V_{DS} > V_{DS_{[SAT]}}$.
\end{tcolorbox}


Para la obtención de $I_{D_{[SAT]}}$, se reemplaza en la expresión (\ref{equation:Expresion_Completa}), $ V_{DS} = V_{DS_{[SAT]}} =  V_{p0} + V_{GS} - V_{bi}$, tal como indica la expresión (\ref{equation:obtension_VDS_SAT_4}), llegando a : 
	
	\begin{equation} \label{equation:ID_SAT_COMPLETA}
		I_{D_{[SAT]}} = g_o \cdot	\left\{[V_{p0} + V_{GS} -  V_{bi}]  - \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[ (V_{bi} - V_{GS} + [V_{p0} + V_{GS} -  V_{bi}] )^{\frac{3}{2}} - ( V_{bi} - V_{GS})^{\frac{3}{2}} \right] \right\} 
	\end{equation}

	\begin{equation} \label{equation:ID_SAT_COMPLETA_2}
		I_{D_{[SAT]}} = g_o \cdot	\left\{[V_{p0} + V_{GS} -  V_{bi}]  - \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[( V_{p0} )^{\frac{3}{2}} - ( V_{bi} - V_{GS})^{\frac{3}{2}} \right] \right\} 
	\end{equation}


	\begin{equation} \label{equation:ID_SAT_COMPLETA_3}
		I_{D_{[SAT]}} = g_o \cdot	\left\{ \frac{ V_{p0}}{3} + V_{GS} -  V_{bi}  + \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[  ( V_{bi} - V_{GS})^{\frac{3}{2}} \right] \right\} 
	\end{equation}



\begin{tcolorbox}[colback=black!5!white, colframe=black!75!black, title=Expresión de la corriente $	I_{D_{[SAT]}} $ (modelo completo)]
	
	
	\begin{equation} \label{equation:Expresion_Completa_IDSAT}
	I_{D_{[SAT]}} = g_o \cdot	\left\{ \frac{ V_{p0}}{3} + V_{GS} -  V_{bi}  + \frac{2}{3 \cdot \sqrt{V_{p0}}} \cdot \left[  ( V_{bi} - V_{GS})^{\frac{3}{2}} \right] \right\}  
	\end{equation}
...
Utilizando las expresiones del modelo clásico de $I_D$ presentadas en la sección \ref{sec:modelo_clasico} y la del modelo completo deducido en la sección \ref{sec:modelo_completo} se pueden llevar a cabo análisis y evaluaciones de las curvas que forman, apreciando de esta forma como el modelo clásico es una buena aproximación a grandes rasgos pero a medida que se contemplan efectos no ideales esta deja de ser valida.  

\section{Simulación de curvas}

Utilizando un código de \textit{Python} detallado en el Apéndice~\ref{sec:apendice_codigo}, se obtienen las curvas características del dispositivo.

\subsection{Curva de transferencia}

Se toma como variable la tensión $V_{GS}$ y se utiliza la tensión $V_{DS} = V_{DS_{[SAT]}}$, obteniendo de esta forma las curvas de la figura \ref{fig:curvatransferencia}:

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Img/curva_transferencia}
	\caption{Curvas de transferencia para el modelo clásico y el modelo completo.}
	\label{fig:curvatransferencia}
\end{figure}

Se puede contemplar como el modelo completo forma una curva tomando valores inferiores en relación del modelo clásico para el rango de estrangulación, mientras que ambas son coincidentes entorno a $V_{GS} = 0$. 


\subsection{Curva de salida}
Para la curva de salida se evaluad $I_D$ en función de la tensión $V_DS$ para un rango de valores de $V_{GS}$¸ se obtienen las curvas de la figura \ref{fig:curvadesalida}:

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Img/curva_de_salida}
	\caption{Curvas de salida para distintos valores de $V_{S}$ para ambos modelos.}
	\label{fig:curvadesalida}
\end{figure}


Se puede observar que en este caso la aproximación clásica es convergente para el valor de $V_{GS} = 0 $ en la región de saturación, para el caso de tensiones de control menores se corresponde con corrientes de saturación mas altas en comparación con las obtenidas por el modelo completo.                                                     
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.45\textwidth}
		\centering
		\includegraphics[width=\linewidth]{Img/modulacion_del_canal}
		\captionof{figure}{Diagrama esquemático del FET cuando $V_{DS} > V_{DS_{[sat]}}$, el extremo de drenaje del canal está comprimido. (b) Diagrama de bandas a lo largo del canal del dispositivo. \parencite{mishra_semiconductor_2008}.}
		\label{fig:modulaciondelcanal}
	\end{minipage}
	\hfill
	% ---------- Texto (derecha) ----------
	\begin{minipage}[c]{0.5\textwidth}
		
			 La longitud del canal n neutro disminuye a medida que $V_{DS}$ aumenta; por lo tanto, la corriente de drenador aumentará, tal como se observa en la figura \ref{fig:modulaciondelcanal}. El cambio en la longitud efectiva del canal y el correspondiente cambio en la corriente de drenador se denomina modulación de la longitud del canal \parencite{neamen2012}.
			
			Tomando la tensión longitudinal cuando $V_{DS} > V_{DS_{[SAT]}}$, se tiene $V(x) \in [0, V_{DS}] $ y se define una tensión extra tal que $V_{DS} =V_{DS_{[SAT]}} + V'$, del mismo modo que se obtuvo el ancho de agotamiento en la expresión ()\ref{equation: W_d_polarizado}), se puede obtener el $\Delta L$ correspondiente con la modulación del canal debido a $V'$.
			
			\begin{equation}
				V' = ( V_{DS} - V_{DS_{[SAT]} }  ) = \frac{q \cdot N_D \dot (\Delta L)^2 }{2 \cdot \epsilon_s}
			\end{equation} 
			
			
			\begin{equation}
				\Delta L = \sqrt{   \frac{2 \cdot \epsilon_s (  V_{DS} - V_{DS_{[SAT]} }   )}{  q \cdot N_D}    }	
			\end{equation} 
			
			De esta forma para la región de saturación del canal, la longitud del canal modula vendria dada por la expresión :
			
			\begin{equation}\label{equation:L_Modulada}
				L' = L - \frac{1}{2} \cdot 	\Delta L
			\end{equation}
...
Por lo tanto la corriente para $V_{DS} > V_{DS_{[SAT]}} $ respondería a la expresión (\ref{equation:ID_SAT_COMPLETA_3}) multiplicada por un factor que ajuste la nueva longitud. Se define el parámetro de modulación de longitud de canal $\lambda $ tomando una pendiente promedio del efecto descripto, dicho efecto se simulara para un valor determinado de $V_{GS}$ y se vera en la curva de salida para valores superiores a $V_{DS_{[sat]}}$ figura \ref{fig:curvadesalidaefectomodulacion}.
	
	\begin{equation}
		I_{D_{[SAT]} } ' = I_{D_{[SAT]} } \cdot \frac{L}{L'}  \approx I_{D_{[SAT]} } \cdot (1 + \lambda \cdot V_{DS})
	\end{equation}


Factorizando $L$ en el denominador:

\begin{equation}
	\frac{L}{L'} =
	\frac{L}{L \left( 1 - \frac{\Delta L}{2L} \right)}
	=
	\frac{1}{1 - \frac{\Delta L}{2L}}
\end{equation}

Si se cumple que $\Delta L \ll L$, puede utilizarse el desarrollo en serie de Taylor:

\begin{equation}
	\frac{1}{1-x} \approx 1 + x \qquad \text{para } |x| \ll 1 \quad \quad 	x = \frac{\Delta L}{2L}
\end{equation}

Se obtiene:

\begin{equation}
	\frac{L}{L'} \approx 1 + \frac{\Delta L}{2L}
\end{equation}

Reemplazando $\Delta L $, finalmente:

\begin{equation}\label{equation:L_Lprima}
	\frac{L}{L'}
	\approx
	1 +
	\frac{1}{2L}
	\sqrt{
		\frac{2 \epsilon_s}{q N_D}
	}
	\sqrt{V_{DS} - V_{DS_{[SAT]}}}	\approx
	1 + \lambda \cdot V_{DS}
\end{equation}



Se puede notar en la expresión (\ref{equation:L_Lprima})  que a medida que el canal tenga menor longitud $L$ (canal corto), el efecto sera mayor con lo cual a medida que $L$ aumenta el efecto se podría despreciar para valores bajos de $V_{DS}$ esto se aprecia también en la simulación para un MESFET con una longitud mayor logrando la curva de color naranja (se asume que el resto de parámetros se ajusten para que coincidan en su valor de $I_{D_{[SAT]}}$).

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{Img/curva_de_salida_efectoModulacion}
	\caption{Curva de salida afectada por la modulación del canal.}
	\label{fig:curvadesalidaefectomodulacion}
\end{figure}


\subsection{Saturación de la velocidad de arrastre}

La velocidad de arrastre de un portador en un semiconductor se satura al aumentar el campo eléctrico, tal como se aprecia en la figura \ref{fig:varrvscampo}. Este efecto de saturación de la velocidad implica que la movilidad no es constante tal como indica la expresión (\ref{equation:mu_definicin}) para el rango lineal de la velocidad en función del campo.
...
\begin{figure}[H]
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\linewidth]{Img/v_arr_vs_campo}
		\caption{Velocidad de arrastre en función del campo aplicado para diferentes semiconductores \parencite{neamen2012}.}
		\label{fig:varrvscampo}
	\end{figure}
	\end{minipage}
	\hfill
	% ---------- Texto (derecha) ----------
	\begin{minipage}[c]{0.5\textwidth}
		...
		\begin{equation}\label{equation:mu_definicin}
			\mu_n = \frac{v_{arr,n}}{\xi } \quad \quad \text{ $v_{arr,n}$: Velocidad de arrastre promedio.}
		\end{equation}
		
		Se aprecia también la figura \ref{fig:varrvscampo} un sobre pico previo a la saturación de la velocidad de arrastre en el caso del GaAs y una región con pendiente negativa para la velocidad de arrastre, esto puede explicarse distinguiendo en el diagrama de la energía de los portadores libres en función del vector de onda, a medida que se incrementa el campo, la banda de conducción de los electrones se excita desde el valle inferior de alta movilidad hasta el valle superior de baja movilidad , incrementando la población de electrones del valle superior a expensas de la del inferior. Esto provoca que la velocidad global de arrastre disminuya , efecto aprovechado para crear amplificadores de potencia de microondas y osciladores locales.
...
Se simulara este efecto tomando como referencia la relación de la figura \ref{fig:varrvscampo} utilizando $v_{sat} = 6.8\cdot 10^6 \, \frac{cm}{s}$ junto con constantes de aproximación y el largo de canal $L$. La curva formada por la corriente $I_D$ en función de $V_{DS}$ permitirá ver este efecto.

\begin{equation}\label{equation:v_arr_enfuncion_de_E}
v_{arr}(\xi) =
\frac{\mu_n \, \xi}{1 + \left(\dfrac{\xi}{\xi_p}\right)^2}
+
v_{sat} \,
\frac{\left(\dfrac{\xi}{\xi_s}\right)^2}{1 + \left(\dfrac{\xi}{\xi_s}\right)^2}\quad \quad,\quad \xi \approx \frac{V_{DS}}{L}
\end{equation}


\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{Img/vel_arrastre_func_campo}
	\caption{}
	\label{fig:velarrastrefunccampo}
\end{figure}

De la velocidad modelada en (\ref{equation:v_arr_enfuncion_de_E}), la cual se basa en el gráfico del libro \parencite{neamen2012} figura \ref{fig:varrvscampo}, se tomara la aproximación de $\mu_n$ constante tomando la pendiente de la curva desde $\xi = 0$ hasta $\xi=\xi_p$ (se comprueba que es cercano a los $8000\, \frac{cm^2}{V\cdot s}$ teóricos) , luego se hará un barrido hasta el valor obtenido en saturación de la velocidad de arrastre comenzando desde el valor de $V_{DS} = (\xi_p \cdot L)$, se obtiene de esta forma la curva de la figura \ref{fig:modulacioncanalmuvariable} que simula el efecto no ideal que se compara con la curva obtenida con $\mu_n$ constante. 


\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{Img/modulacion_canal_mu_variable}
	\caption{Curva de salida semilogaritmica con efectos no ideales para canal corto y canal largo.}
	\label{fig:modulacioncanalmuvariable}
\end{figure}
...
\subsection{Región subumbral}
	...
	La corriente subumbral es un efecto no ideal que ocurre en la región $ V_{GS} < V_p$ observable en la figura \ref{fig:corrientesubumbral}, la aproximación de agotamiento abrupto no modela con precisión la región del canal es por eso que la ecuación (\ref{equation:ID_SAT_COMPLETA}) no describe adecuadamente los resultados experimentales cerca del voltaje umbral. Esta corriente se debe a la onducción parcial en la región no totalmente vaciada, corrientes de difusión de portadores y corriente termoiónica sobre la barrera Schottky. Se simulo una corriente subumbral de corriente máxima $I_0$ en la figura \ref{fig:corrientesubumbral_simulacion}.
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.45\textwidth}
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.85\linewidth]{"../../../../Imágenes/Capturas de pantalla/corriente_sub_umbral"}
			\caption{Ejemplo gráfico de corriente subumbral contrastada con la curva ideal \parencite{neamen2012}.}
			\label{fig:corrientesubumbral}
		\end{figure}
	\end{minipage}
	\hfill
	% ---------- Imagen (derecha) ----------
	\begin{minipage}[c]{0.5\textwidth}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\linewidth]{Img/corriente_subumbral_simulacion}
				\caption[]{Simulación de corriente subumbral de característica exponencial con magnitud del orden de $10^{-9}$ A.}
				\label{fig:corrientesubumbral_simulacion}
			\end{figure}
		\end{minipage}
...
Nuevamente dejando de lado los modelos ideales, donde la corriente en el terminal \textit{gate} es cero ($I_G = 0 $), en la unión Schottky existen corrientes de compuerta apreciables o no de acuerdo a la sensibilidad de la implementación y las condiciones de funcionamiento, estas son debido a fenómenos como tunealamiento, emisión termoionica, corrientes de ruptura y generación y recombinación de portadores. 
En esta situación tenemos que tener en cuenta la generación térmica de pares electrón-hueco. Los pocos electrones generados térmicamente pierden energía y bajan del metal a el canal n, siendo una corriente inversa $I_s$ que es muy pequeña. Estos fenómenos se incrementan con la temperatura, por lo tanto se simulo el efecto para distintos valores de temperatura figura \ref{fig:efectosdecorrientesgate} (tomando como referencia las corrientes en una unión Schottky en inversa que presentaría un diodo en función de la temperatura (\ref{equation:corriente_inversa_diodo_Schottky}), con $V = V_G - V(x)$ ).


\begin{equation}\label{equation:corriente_inversa_diodo_Schottky}
	I_G(T) = - I_S(T)\left[\exp\left(\frac{qV}{n k T}\right) - 1\right] \quad \wedge \quad	I_S(T) = T^{2} \exp\left(-\frac{\phi_B}{kT}\right) 
\end{equation}
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.4\textwidth}
		\begin{figure}[H]
			\centering
			\includegraphics[width=1\linewidth]{Img/efectos_corr_gate.png}
			\caption{Medida de corriente en función de $V_{G}$ para un MESFET de GaAs, mostrando la corriente $I_D$ para tres regiones normal, subumbral y corrientes de compuerta \parencite{neamen2012}.}
			\label{fig:efectos_corr_gate}
		\end{figure}
	\end{minipage}
	\hfill
	% ---------- Texto (derecha) ----------
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\linewidth]{Img/efectos_de_corrientes_gate}
		\caption{Corrientes de fuga en una unión metal semiconductor en inversa.}
		\label{fig:efectosdecorrientesgate}
	\end{figure}
...
\section{Tensión de ruptura }
Para los MESFET de GaAs, los mecanismos de ruptura son mucho más complejos que con los JFET. Generalmente presentan valores de ruptura ($V_{B}$) más bajos tal como se ve en la figura \ref{fig:tensionderuptura}. Un factor que reduce la tensión de ruptura se debe a la corriente de túnel asociada al contacto de la compuerta de la barrera Schottky. En campos altos, esta corriente de túnel proviene de la emisión del campo termoiónico, que depende de la temperatura. La corriente de compuerta puede iniciar la multiplicación por avalancha e induce una menor tensión de ruptura del drenador. 
...
\begin{figure}[H]
	...
	% ---------- Imagen (izquierda) ----------
	\begin{minipage}[c]{0.4\textwidth}
		Con una corriente de canal más alta, el nodo interno se encuentra a una temperatura más alta, lo que desencadena una ruptura por avalancha más temprana, iniciada por la corriente de compuerta. Esto puede ser responsable de una $V_{GS}$ menor a una $V_{GS}$ mayor, como se muestra en la Fig. \ref{fig:tensionderuptura}. Otro factor es que los MESFET de GaAs suelen tener mayor corriente y transconductancia que los dispositivos de Si debido a su mayor movilidad. La mayor corriente de canal puede iniciar la avalancha a una tensión más baja o producir los efectos de temperatura que desencadenan una ruptura más temprana, como se mencionó. La tensión de ruptura se puede mejorar ampliando la región entre la compuerta y el drenador \parencite{sze2021}..
	...
	\begin{minipage}[c]{0.48\textwidth}
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth]{Img/Tension_de_Ruptura}
			\caption{Datos experimentales que muestran que la tensión de ruptura del drenador (a) aumenta con $V_{GS}$, en JFET de Si, pero (b) disminuye con $V_{GS}$, en MESFET de GaAs \parencite{sze2021}.}
			\label{fig:tensionderuptura}
		\end{figure}
...
El transistor MESFET ejemplifica como el estudio de los semiconductores y los fenómenos físicos entorno a ellos es de importancia para dar con dispositivos adecuados para distintos escenarios y rendimientos objetivos. Los conocimientos teóricos adquiridos en la materia ayudaron a comprender su funcionamiento, mientras que los prácticos ayudaron a deducir las expresiones analíticas partiendo de las relaciones físicas iniciales y dar con simulaciones adecuadas. La deducción de la corriente mediante el modelo completo permitió comparar con el modelo clásico y verificar en que escenarios se puede considerar valido.  

El modelo clásico es conveniente para la aplicación de transistores en aplicaciones que requieran poca sensibilidad, tensiones bajas de operación y frecuencias que no lleven al limite el modelo. Mientras que el modelo completo se deberá contemplar a la hora del diseño de los mismos transistores y circuitos integrados como también los efectos no ideales tratados en las ultimas secciones del informe. A su vez se resalta como la longitud del canal y la temperatura de operación son de importancia para el comportamiento del dispositivo.
...
\begin{verbatim}
		...
		label_text = (
		r"MESFET (GaAs / Ti)" "\n"
		r"_ Modelo con efecto de modulacion" "\n"
		...
		plt.xscale("log")
		plt.yscale("log")
		plt.xlabel(r"Campo eléctrico $\xi$ [V/cm]")
		...
		plt.plot(VDS, ID_cste*1e3, label=r"Modelo $\mu$ constante",alpha=0.8, linewidth = 4,color="blue", linestyle= "--")
		plt.plot(VDS, ID*1e3, label=r"Modelo  $\mu$ variable", linewidth = 4, color ="red")
		
		plt.axvline(V1, linestyle="--", alpha=0.5, label= rf"Inicio caída $\mu$: $V_{{DS}} = \xi_p \cdot L  = $ {V1:.1f}",
		linewidth = 3)
		...
		plt.text(0.1, 0.65, label_text,transform=plt.gca().transAxes,fontsize=10,verticalalignment='top',bbox=dict(boxstyle="round",
		...
		plt.text(0.05, 0.4,label_text,transform=plt.gca().transAxes,fontsize=12,verticalalignment='top'
		,bbox=dict(boxstyle="round",facecolor="white", alpha=0.85))
