...
Se definen la barrera de potencial entre el metal y el semiconductor $\Phi_B$ = ($\Phi_M - \chi$) y barrera de juntura $\Phi_{bi}$ = $\Phi_B - \frac{1}{q} (E_c - E_f)$, también se puede apreciar como la flexión determina la zona de vaciamiento $W_d$ en equilibrio definida como $W_d0$. A su vez, se obtiene la tensión de juntura : $ V_bi = \phi_{bi} = \Phi_{M} - \chi_{sc} $. 
