...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\linewidth]{Img/simbolo_mas_esquema}
...
Nuevamente dejando de lado los modelos ideales, donde la corriente en el terminal \textit{gate} es cero ($I_G = 0 $), en la unión schottky  existen corrientes de compuerta apreciables o no de acuerdo a la sensibilidad de la implementación y las condiciónes de funcionamiento, estas son debido a fenómenos como tunealamiento, emisión termoionica, corrientes de ruptura y generación y recombinación de portadores. 
En esta situación tenemos que tener en cuenta la generación térmica de pares electrón-hueco. Los pocos electrones generados térmicamente pierden energía y bajan del metal a el canal n, es una "Corriente Inversa" (IS) que es muy pequeña. Estos fenomenos se incrementan con la temperatura, por lo tanto se simulo el efecto para distintos valores de temperatura figura \ref{fig:efectosdecorrientesgate} (tomando como referencia las corrientes en una unión Schottky en inversa que presentaría un diodo).
...
\begin{figure}[H]
	...
	\begin{minipage}[c]{0.4\textwidth}
		\begin{figure}[H]
			...
			\caption{Medida de corriente en función de $V_{GS}$ para un MESFET de GaAs, mostrando la corriente $I_D$ para tres regiones normal, subumbral y corrientes de compuerta \parencite{neamen2012}.}
...
\begin{verbatim}
		...
		plt.axvline(V1, linestyle="--", alpha=0.5, label= rf"Inicio caída $\mu$: $V_{{DS}} = \xi_p \cdot L  = $ {V1:.1f}", linewidth = 3)
