...
Al diseñar un MESFET canal n, se puede optar por una determinada concentración de impurezas y geometría del canal tal que requiera tensiones de compuerta negativas para modular o interrumpir la conducción, el cual se conoce como modo empobrecimiento (D-MESFET o Depletion-mode MESFET) o tensiones de control positivas, este ultimo nombrado como modo enriquecimiento (E-MESFET o Enhancement-mode MESFET). De esta forma se pueden clasificar en dos tipos de MESFET por cada tipo de canal, los mismos comúnmente se diferencian con ayuda de los símbolos que se presentan en la tabla de la figura (\ref{fig:tiposmesfet}).
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{Img/Tipos_MESFET}
	\caption{ Tabla resumen de las características de cada tipo de MESFET de canal n \parencite{BoylestadNashelsky2015}.}
...
En el informe se tomara como objeto de estudio un transistor MESFET formado por GaAs de canal n de empobrecimiento (D-MESFET), con silicio como impureza dopante en el canal y titanio como el metal de la compuerta a una temperatura de 300 K. A continuación se presentaran los diferentes parámetros de referencia utilizados en el informe.
...
Cuando un solido no es impurificado, se dice que es un material intrínseco, al añadirle impurezas el diagrama de bandas de este solido se modifica de acuerdo a la concentración y el tipo de estas impurezas. A su vez un semiconductor en el que los huecos y los electrones se crean exclusivamente mediante una excitación térmica a través de la banda prohibida de energía, se conoce como semiconductor intrínseco \parencite{neamen2012}.

Como ya se menciono el MESFET se forma principalmente con metal (conductor), semiconductor dopado tipo n, y un semi-aislante (Semiconductor GaAs con Cromo como impureza).




\subsubsection{Conductor: Titanio}
...
\begin{tikzpicture}[
	...
	% Ejes
	\draw[axis] (0,-3) -- (0,2.5) node[above] {Energía};
	
	% Nivel de vacío
	\draw[level] (-2.5,2) -- (2.5,2);
	\node[annot, right] at (2.5,2) {$E_0$};
	
	% Nivel de Fermi
	\draw[level] (-2.5,0) -- (2.5,0);
	\node[annot, right] at (2.5,0) {$E_f$};
	
	% Banda metálica (ocupada)
	\fill[gray!40] (-2.5,-2) rectangle (2.5,0);
	\node[annot] at (0,-1) {Estados ocupados};
	
	% Banda metálica (desocupada)
	\fill[gray!20] (-2.5,0) rectangle (2.5,1.2);
	\node[annot] at (0,0.6) {Estados vacíos};
	
	% Función trabajo
	\draw[<->, thick] (-2.2,0) -- (-2.2,2);
	\node[annot, left] at (-2.2,1) {$\phi_m$};
	
	% Etiqueta material
	\node[annot] at (0,-4) {Metal: Titanio (Ti)};
...
\begin{tikzpicture}[
	...
	% Ejes
	\draw[axis] (-3,0) -- (3,0) node[right] {posición};
	\draw[axis] (0,-3) -- (0,2.8) node[above] {Energía};
	
	% Nivel de vacío
	\draw[level] (-2.5,2.3) -- (2.5,2.3);
	\node[annot, right] at (2.5,2.3) {$E_0$};
	
	% Banda de conducción
	\draw[band] (-2.5,1.0) -- (2.5,1.0);
	\node[annot, right] at (2.5,1.0) {$E_c$};
	
	% Banda de valencia
	\draw[band] (-2.5,-1.2) -- (2.5,-1.2);
	\node[annot, right] at (2.5,-1.2) {$E_v$};
	
	% Nivel de Fermi (tipo N, cercano a Ec)
	\draw[level] (-2.5,0.7) -- (2.5,0.7);
	...
	\draw[<->, thick] (2.1,1.0) -- (2.1,2.3);
	\node[annot, right] at (2.1,1.65) {$\chi$};
...
\begin{tikzpicture}[
	...
	band/.style={thick},
	level/.style={dashed, thick},
	deep/.style={dotted, thick},
	annot/.style={font=\small}
	]
	
	% Ejes
	\draw[axis] (-3,0) -- (3,0) node[right] {posición};
	\draw[axis] (0,-3) -- (0,3) node[above] {Energía};
	
	% Nivel de vacío
	\draw[level] (-2.5,2.6) -- (2.5,2.6);
	...
	% Banda de valencia
	\draw[band] (-2.5,-1.4) -- (2.5,-1.4);
	\node[annot, right] at (2.5,-1.4) {$E_v$};
	
	% Nivel profundo del Cr
	\draw[deep] (-2.5,-0.1) -- (2.5,-0.1);
	\node[annot, right] at (2.5,-0.1) {$E_{Cr}$};
	
	% Nivel de Fermi (casi centrado en el gap)
	\draw[level] (-2.5,-0.2) -- (2.5,-0.2);
	\node[annot, right] at (2.5,-0.2) {$E_f$};
	
	% Gap
	\draw[<->, thick] (-2.1,-1.4) -- (-2.1,1.2);
	\node[annot, left] at (-2.1,-0.1) {$E_g$};
	
	% Afinidad electrónica
	\draw[<->, thick] (2.1,1.2) -- (2.1,2.6);
	\node[annot, right] at (2.1,1.9) {$\chi$};
	
	% Etiqueta material
	\node[annot] at (0,2.85) {Semiconductor: GaAs semi-aislante (Cr)};
